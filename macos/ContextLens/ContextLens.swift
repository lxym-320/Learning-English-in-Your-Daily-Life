...
@MainActor final class SelectionMonitor: ObservableObject {
    ...
    func retrySelection(id:Int,delays:[Double]){guard id==attemptID,let delay=delays.first else{if id==attemptID{last="";onDeselection?()};return};DispatchQueue.main.asyncAfter(deadline:.now()+delay){guard id==self.attemptID,!self.mouseIsDown else{return};if !self.readSelection(deep:true){self.retrySelection(id:id,delays:Array(delays.dropFirst()))}}}
    ...
    func selectedText(from element:AXUIElement)->String? {
        var value:CFTypeRef?
        if AXUIElementCopyAttributeValue(element,kAXSelectedTextAttribute as CFString,&value) == .success,let text=value as? String,!text.isEmpty{return text}
        var range:CFTypeRef?
        if AXUIElementCopyAttributeValue(element,kAXSelectedTextRangeAttribute as CFString,&range) == .success,let range {value=nil;if AXUIElementCopyParameterizedAttributeValue(element,kAXStringForRangeParameterizedAttribute as CFString,range,&value) == .success,let text=value as? String,!text.isEmpty{return text}}
        var markerRange:CFTypeRef?
        if AXUIElementCopyAttributeValue(element,"AXSelectedTextMarkerRange" as CFString,&markerRange) == .success,let markerRange {for attribute in ["AXStringForTextMarkerRange","AXAttributedStringForTextMarkerRange"]{value=nil;if AXUIElementCopyParameterizedAttributeValue(element,attribute as CFString,markerRange,&value) == .success{if let text=value as? String,!text.isEmpty{return text};if let text=value as? NSAttributedString,!text.string.isEmpty{return text.string}}}}
        return nil
    }
    func searchChromeTree(_ root:AXUIElement)->String? {var queue=[root];var checked=0;while !queue.isEmpty,checked<180{let element=queue.removeFirst();checked += 1;if let text=selectedText(from:element){return text};var children:CFTypeRef?;if AXUIElementCopyAttributeValue(element,kAXChildrenAttribute as CFString,&children) == .success,let list=children as? [AXUIElement]{queue.append(contentsOf:list.prefix(24))}};return nil}
    func readSelection(deep:Bool=false)->Bool {guard AXIsProcessTrusted() else{lastDiagnostic="辅助功能权限未生效";return false};let system=AXUIElementCreateSystemWide();var appValue:CFTypeRef?;var focusedValue:CFTypeRef?;_=AXUIElementCopyAttributeValue(system,kAXFocusedApplicationAttribute as CFString,&appValue);if AXUIElementCopyAttributeValue(system,kAXFocusedUIElementAttribute as CFString,&focusedValue) != .success || focusedValue == nil,let appValue{_=AXUIElementCopyAttributeValue(appValue as! AXUIElement,kAXFocusedUIElementAttribute as CFString,&focusedValue)};guard let focusedValue else{lastDiagnostic="当前应用没有可读取的文本区域";return false};var element=focusedValue as! AXUIElement;for _ in 0..<10{if let text=selectedText(from:element),accept(text){return true};var parent:CFTypeRef?;guard AXUIElementCopyAttributeValue(element,kAXParentAttribute as CFString,&parent) == .success,let parent else{break};element=parent as! AXUIElement};if deep,let appValue{var window:CFTypeRef?;if AXUIElementCopyAttributeValue(appValue as! AXUIElement,kAXFocusedWindowAttribute as CFString,&window) == .success,let window,let text=searchChromeTree(window as! AXUIElement),accept(text){return true}};lastDiagnostic="当前应用未开放普通选区或网页 Text Marker";return false}
}
...
@MainActor final class AppDelegate:NSObject,NSApplicationDelegate,NSWindowDelegate {
    ...
    @objc func diagnoseSelection(){let success=monitor.readSelection(deep:true);let alert=NSAlert();alert.messageText=success ? "选词读取正常":"选词读取失败";alert.informativeText=monitor.lastDiagnostic+(success ? "\n已在鼠标附近显示翻译按钮。":"\n请先在 Chrome 或文本编辑器中选中一段英文，再立即点击菜单栏 Context → 诊断当前选词。");alert.runModal()}
}
